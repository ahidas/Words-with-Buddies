...
export function GameBoard({vals, setVals}){
  ...
  for (let i = 0; i < 15; i++){
    board.push(<BuildRow vals={vals} index={i} setVals={setVals} inputRefs = {inputRefs} setCurrCor={setCurrCor} />);
  }
  useEffect(() => {
    if(currCor[0] === -1){
      return;
...
function BuildRow({ vals, index, setVals,inputRefs,setCurrCor }){
  const rows = [];
  let type;
  ...
  ///double word
   ...
   // console.log(type);
    const newCol = <input onChange={(e) => changeHandler(i,e.target.value)} className={type} key={boxKey}  ref={(el) => (inputRefs.current[index][i] = el)} value={vals[index][i]} x = {i} y={index}  />
    boxKey++;
    rows.push(newCol);
