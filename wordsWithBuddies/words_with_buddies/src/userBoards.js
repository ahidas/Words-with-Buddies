...
export function UserBoards( { setVals, emptyBoard, curr_board }){
    ...
    async function set_board(board){
        const result = await fetch("http://127.0.0.1:5006/api/get_board",{method: 'POST', credentials: 'include' , body: board});
    ...
    async function getBoardNames(){
        const result = await fetch("http://127.0.0.1:5006/api/get_board_names",{method: 'GET', credentials: 'include'});
        const js = await result.json();
        console.log(js);
        setBoardNames(js);   
    }
