...
export function UserForm( {loggedIn, setLoggedIn, vals, curr_board} ){
    ...
    async function save_board(){
      ...
      const result = await fetch("https://ahidas.pythonanywhere.com/api/save_board",{method: 'POST', credentials: 'include', body: JSON.stringify(data)});
...
function SignUp( { setShowSignUp, setLoggedIn, curr_board }){
    ...
    async function handleSubmit(e){
        ...
        const data = JSON.stringify({"username": Username.current, "password": Password.current});
          //getting updates
        const result = await fetch("https://ahidas.pythonanywhere.com/api/sign_up",{method: 'POST', credentials: 'include', body: data});
        const js = await result.json();
        console.log(js);
        if(js === true){
...
function Login( { setShowLogin, setLoggedIn }){
    ...
    async function handleSubmit(e){
        ...
        const data = JSON.stringify({"username": Username.current, "password" : Password.current});
          //getting updates
        const result = await fetch("https://ahidas.pythonanywhere.com/api/login",{method: 'POST', credentials: 'include', body: data});
        const js = await result.json();
        console.log(js);
        if(js === true){
